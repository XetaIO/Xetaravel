                    ...
                    @forelse($permissions as $permission)
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $permission->getKey() }}">
                            ...
                            @can('update', $permission)
                                <x-table.cell>
                                    <x-button icon="fas-pen-to-square" data-content="{{ $permission->getKey() }}" class="$permissionUpdateButton btn-ghost tooltip tooltip-right" data-tip="Edit this category" />
                                </x-table.cell>
                            @endcan
                            <x-table.cell>
                                {{ $permission->getKey() }}
