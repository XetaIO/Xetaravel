<div>
    ...
    <x-table.table class="mb-6">
        <x-slot name="head">
            ...
            <x-table.heading sortable wire:click="sortBy('conversation_count')" :direction="$sortField === 'conversation_count' ? $sortDirection : null">Conversations</x-table.heading>
            <x-table.heading sortable wire:click="sortBy('is_locked')" :direction="$sortField === 'is_locked' ? $sortDirection : null">Locked</x-table.heading>
        ...
        <x-slot name="body">
            ...
            @forelse($categories as $category)
                <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $category->getKey() }}">
                    ...
                    <x-table.cell><i class="{{ $category->icon }}"></i></x-table.cell>
                    <x-table.cell class="font-bold" style="color: {{ $category->color }}">{{ $category->title }}</x-table.cell>
                    <x-table.cell>{{ $category->level }}</x-table.cell>
                    <x-table.cell>{{ $category->description }}</x-table.cell>
                    <x-table.cell>{{ $category->conversation_count }}</x-table.cell>
                    <x-table.cell>
    ...
    <form wire:submit.prevent="save">
        ...
        <label for="editModal" class="modal cursor-pointer">
            <label class="modal-box relative">
                ...
                <x-form.input-group wire:model="model.color" wire:keyup='generateColor' id="color" name="model.color" type="text" label="Color" placeholder="#dddddd" style="color:{{ $color }}">
                    <span class="h-4 w-4" style="background-color:{{ $color }}"></span>
                </x-form.input-group>
