<div>
    ...
    <x-modal wire:model="showModal" title="Create a Discussion" class="backdrop-blur" box-class="w-11/12 max-w-5xl">
        ...
        @can('pin', \Xetaravel\Models\DiscussConversation::class)
            <x-checkbox wire:model.live="form.is_pinned" name="form.is_pinned" label="Check to pin this discussion" />
        @endcan

        @can('lock', \Xetaravel\Models\DiscussConversation::class)
            <x-checkbox wire:model.live="form.is_locked" name="form.is_locked" label="Check to lock this discussion" />
        @endcan
