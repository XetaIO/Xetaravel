                ...
                <div @class(["w-full", "join" => $prepend || $append])>
                    ...
                    >
                            ...
                            <span wire:key="selected-options-{{ $uuid }}">
                                ...
                                @else
                                    <template x-for="(option, index) in selectedOptions" :key="index">
                                                        <span class="choices-element cursor-pointer badge badge-soft m-0.5 !inline-block">
                                                            {{-- SELECTION SLOT --}}
                                                            @if($selection)
                                                            ...
                                                            <x-icon @click="toggle(option.{{ $optionValue }})" x-show="!isReadonly && !isDisabled && !isSingle" name="fas-xmark" class="w-4 h-4 hover:text-error" />
                                                        </span>
                                                    </template>
                                @endif
                                            </span>
                        ...
                        @if($clearable && !$isReadonly() && !$isDisabled())
                            <x-icon @click="reset()" x-show="!isSelectionEmpty" name="fas-xmark" class="cursor-pointer w-4 h-4 opacity-40"/>
                        @endif
