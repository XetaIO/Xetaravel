<div wire:key="{{ $uuid }}">
    ...
    >
        ...
        <!-- CONTENT -->
        <div class="flex-1 overflow-hidden whitespace-nowrap text-ellipsis truncate w-0 hideable">
            @if($link)
                <a href="{{ $link }}" wire:navigate>
                    @endif
        ...
        @if($actions)
            @if($link && !Str::of($actions)->contains([':click', '@click' , 'href']))
                <a href="{{ $link }}" wire:navigate>
                    @endif
                    <div {{ $actions->attributes->class(["py-3 flex items-center gap-3 hideable"]) }}>
                        {{ $actions }}
                    </div>
