...
<article class="md:max-w-none grid lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12 xl:gap-16 items-center bg-base-200 dark:bg-base-300 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-10">
    ...
    <div>
        <header>
            <h3 class="text-2xl lg:text-3xl font-bold leading-tight mb-2">
                <a class="link link-hover link-primary" href="{{ $article->show_url }}">
        ...
        <footer class="flex items-center mt-4 gap-2">
            ...
            <div class="flex flex-wrap gap-1">
                ...
                <span class="text-gray-500"> - </span>
                    <a class="badge badge-outline badge-primary tooltip" href="{{ $article->category->show_url }}" data-tip="Category">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 inline-block align-text-top">
...
@empty
    <x-alert type="error" class="mb-4">
        ...
        <div>
            @if(request()->route()->getName() === 'blog.article.index')
                <x-button link="{{ route('discuss.index') }}" icon="far-comments" label="Visit Discuss" class="btn-primary btn-sm gap-2" />
            @else
                <x-button link="{{ route('blog.article.index') }}" icon="heroicon-o-newspaper" label="Visit Blog" class="btn-primary btn-sm gap-2" />
            @endif
